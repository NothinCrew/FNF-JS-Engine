...
import funkin.Note.EventNote;
import openfl.events.KeyboardEvent;
import flixel.util.FlxSave;
import funkin.Achievements;
import funkin.StageData;
import funkin.FunkinLua;
import funkin.DialogueBoxPsych;
import funkin.Conductor.Rating;
import funkin.Character.Boyfriend;
import funkin.Shaders;
import funkin.Note.PreloadedChartNote;
import funkin.utils.*;
...
class PlayState extends MusicBeatState
{
	...
	public var grpOptionsHoldSplashes:FlxTypedGroup<SustainSplash>;
	public var grpOptionsNoteSplashes:FlxTypedGroup<NoteSplash>;
	...
	override public function create()
	{
		...
		grpOptionsHoldSplashes = new FlxTypedGroup<SustainSplash>((ClientPrefs.maxSplashLimit != 0 ? ClientPrefs.maxSplashLimit : 10000));
		grpOptionsNoteSplashes = new FlxTypedGroup<NoteSplash>((ClientPrefs.maxSplashLimit != 0 ? ClientPrefs.maxSplashLimit : 10000));
		...
		switch (curStage)
		{
			case 'stage': new funkin.stages.StageWeek1(); //Week 1
			case 'spooky': new funkin.stages.Spooky(); //Week 2
			case 'philly': new funkin.stages.Philly(); //Week 3
			case 'limo': new funkin.stages.Limo(); //Week 4
			case 'mall': new funkin.stages.Mall(); //Week 5 - Cocoa, Eggnog
			case 'mallEvil': new funkin.stages.MallEvil(); //Week 5 - Winter Horrorland
			case 'school': new funkin.stages.School(); //Week 6 - Senpai, Roses
			case 'schoolEvil': new funkin.stages.SchoolEvil(); //Week 6 - Thorns
			case 'tank': new funkin.stages.Tank(); //Week 7 - Ugh, Guns, Stress
			case 'phillyStreets': new funkin.stages.PhillyStreets(); 	//Weekend 1 - Darnell, Lit Up, 2Hot
			case 'phillyBlazin': new funkin.stages.PhillyBlazin();	//Weekend 1 - Blazin
			case 'phillyStreetsBF': new funkin.stages.PhillyStreetsBF(); //Darnell (BF Mix)
		}

		if (Paths.formatToSongPath(SONG.song) == 'stress')
			GameOverSubstate.characterName = 'bf-holding-gf-dead';

		if (Note.globalRgbfunkin.shaders.length > 0) Note.globalRgbShaders = [];
		...
		add(grpOptionsNoteSplashes);
		add(grpOptionsHoldSplashes);
		...
		grpOptionsNoteSplashes.cameras = [camHUD];
		grpOptionsHoldSplashes.cameras = [camHUD];
  ...
  public function removeShaderFromCamera(cam:String,effect:ShaderEffect){
	switch(cam.toLowerCase()) {
		case 'camhud' | 'hud':
			...
			camHUD.filters = newCamEffects;
		case 'camother' | 'other':
			camOther.shaders.remove(effect);
			var newCamEffects:Array<BitmapFilter>=[];
			for(i in camOtherShaders){
				newCamEffects.push(new ShaderFilter(i.shader));
			}
  ...
  public function clearShaderFromCamera(cam:String){
	...
	public function spawnHoldSplash(note:Note) {
		var end:Note = note;
		var splash:SustainSplash = grpOptionsHoldSplashes.recycle(SustainSplash);
		splash.setupSusSplash((note.mustPress ? playerStrums : opponentStrums).members[note.noteData], note, playbackRate);
		grpOptionsHoldSplashes.add(end.noteHoldSplash = splash);
	}
	...
	public function spawnNoteSplash(x:Float, y:Float, data:Int, ?note:Note = null) {
		var splash:NoteSplash = grpOptionsNoteSplashes.recycle(NoteSplash);
