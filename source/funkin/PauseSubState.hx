...
import funkin.GameplayChangersSubstate;

class PauseSubState extends MusicBeatSubstate
{
	var grpOptionsMenuShit:FlxTypedGroup<Alphabet>;
	...
	override function create()
	{
		...
		grpOptionsMenuShit = new FlxTypedGroup<Alphabet>();
		add(grpOptionsMenuShit);
	...
	override function update(elapsed:Float)
	{
		...
		if (accepted && (cantUnpause <= 0 || !ClientPrefs.controllerMode))
		{
			...
			switch (daSelected)
			{
				...
				case 'Chart Editor':
					FlxG.switchState(funkin.editors.ChartingState.new);
				...
				case "Options":
					FlxG.switchState(funkin.OptionsState.new);
					inPause = true;
	...
	function changeSelection(change:Int = 0):Void
	{
		...
		for (item in grpOptionsMenuShit.members)
		{
	...
	public function regenMenu():Void {
		for (i in 0...grpOptionsMenuShit.members.length) {
			var obj = grpOptionsMenuShit.members[0];
			obj.kill();
			grpOptionsMenuShit.remove(obj, true);
			obj.destroy();
		}
