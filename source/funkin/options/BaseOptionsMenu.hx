...
class BaseOptionsMenu extends MusicBeatSubstate
{
	...
	private var grpOptionsTexts:FlxTypedGroup<AttachedText>;
	...
	public function new()
	{
		...
		grpOptionsTexts = new FlxTypedGroup<AttachedText>();
		add(grpOptionsTexts);
	...
	function regenList() {
			...
			grpOptionsTexts.forEach(text -> {
				grpOptionsTexts.remove(text, true);
				text.destroy();
			});
			...
			if(controls.RESET)
			{
				if (!FlxG.keys.pressed.SHIFT) 
				{
					...
					if(leOption.type != 'bool')
					{
						...
						{
							leOption.curOption = leOption.indexOf(leOption.getValue());
						}
