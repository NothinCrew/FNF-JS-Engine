	...
	function openSelectedSubstate(label:String) {
		switch(label) {
			case 'Note Colors':
				...
				else openSubState(new funkin.options.NotesSubState());
			case 'Controls':
			...
			case 'Optimization':
				openSubState(new funkin.options.OptimizationSubState());
			case 'Game Rendering':
				openSubState(new funkin.options.GameRendererSettingsSubState());
			case 'Adjust Delay and Combo':
				LoadingState.loadAndSwitchState(() -> new funkin.options.NoteOffsetState());
			case 'Misc':
				openSubState(new funkin.options.MiscSettingsSubState());
		}
	}
        ...
        if (FlxG.keys.justPressed.ANY) {
            ...
            if (FlxG.keys.anyJustPressed([k])) {
                ...
                if (kId >= keys.length) {
					...
					FlxG.camera.angle = 0;
                        openSubState(new funkin.options.SuperSecretDebugMenu());
                    }});
                }
            }
        }
