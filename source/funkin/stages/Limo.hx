...
{
	var grpOptionsLimoDancers:FlxTypedGroup<BackgroundDancer>;
	...
	var bgLimo:BGSprite;
	var grpOptionsLimoParticles:FlxTypedGroup<BGSprite>;
	...
	override function create()
	{
		...
		if(!ClientPrefs.lowQuality) {
			...
			grpOptionsLimoDancers = new FlxTypedGroup<BackgroundDancer>();
			add(grpOptionsLimoDancers);

			for (i in 0...5)
			{
				var dancer:BackgroundDancer = new BackgroundDancer((370 * i) + dancersDiff + bgLimo.x, bgLimo.y - 400);
				dancer.scrollFactor.set(0.4, 0.4);
				grpOptionsLimoDancers.add(dancer);
			}

			limoLight = new BGSprite('gore/coldHeartKiller', limoMetalPole.x - 180, limoMetalPole.y - 80, 0.4, 0.4);
			add(limoLight);

			grpOptionsLimoParticles = new FlxTypedGroup<BGSprite>();
			add(grpOptionsLimoParticles);

			//PRECACHE BLOOD
			var particle:BGSprite = new BGSprite('gore/stupidBlood', -400, -400, 0.4, 0.4, ['blood'], false);
			particle.alpha = 0.01;
			grpOptionsLimoParticles.add(particle);
			resetLimoKill();
	...
	{
		if(!ClientPrefs.lowQuality) {
			grpOptionsLimoParticles.forEach(function(spr:BGSprite) {
				if(spr.animation != null && spr.animation.curAnim.finished) {
					spr.kill();
					grpOptionsLimoParticles.remove(spr, true);
					spr.destroy();
				}
			});
			...
			switch(limoKillingState) {
				case KILLING:
					...
					var dancers:Array<BackgroundDancer> = grpOptionsLimoDancers.members;
	...
	{
		if(!ClientPrefs.lowQuality) {
			grpOptionsLimoDancers.forEach(function(dancer:BackgroundDancer)
			{
				dancer.dance();
			});
		}
