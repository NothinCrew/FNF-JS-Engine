...
class MenuCharacterEditorState extends MusicBeatState
{
	...
	function addCharacterUI() {
		...
		{
			grpOptionsWeekCharacters.members[curTypeSelected].flipX = flipXCheckbox.checked;
	...
	function updateCharacters() {
		for (i in 0...3) {
			var char:MenuCharacter = grpOptionsWeekCharacters.members[i];
			char.alpha = 0.2;
			char.character = '';
	...
	function reloadSelectedCharacter() {
		var char:MenuCharacter = grpOptionsWeekCharacters.members[curTypeSelected];
	...
	override function update(elapsed:Float) {
		...
		if(!blockInput) {
			...
			if(FlxG.keys.justPressed.ESCAPE) {
				FlxG.switchState(funkin.editors.MasterEditorMenu.new);
			...
			if(FlxG.keys.justPressed.SPACE && curTypeSelected == 1) {
				grpOptionsWeekCharacters.members[curTypeSelected].animation.play('confirm', true);
			}
		}

		var char:MenuCharacter = grpOptionsWeekCharacters.members[1];
