	...
	override function update(elapsed:Float)
	{
		...
		if (!blockInput)
		{
			if (FlxG.keys.justPressed.ESCAPE)
			{
				...
				FlxG.sound.music.pause();
				pauseVocals();
				LoadingState.loadAndSwitchState(() -> new funkin.editors.EditorPlayState(sectionStartTime()));
			...
			if (FlxG.keys.justPressed.BACKSPACE) {
				...
				openSubState(new Prompt('WARNING! This action will clear unsaved progress.\n\nProceed?', 0, 
					function() FlxG.switchState(funkin.editors.MasterEditorMenu.new), null,ignoreWarnings));
			}
